  ...
  <div class="success">
    ...
    <div class="succes-box">
      <div class="succes-top">
        ...
        <div class="jieshao"> 尊敬的用户您已经成功订购 <span class="blue">{{name}},{{miaoshu
          }}</span> 感谢您的使用!</div>
...
export default {
    ...
    getPaymentDetail (acctId) {
      ...
      getPaymentDetail({ acctId }).then(res => {
        ...
        if (res.data.respCode === '0000') {
         this.money = res.data.data.generalBalance
          console.log(this.money,'成功后的钱')
          if(this.money<0){
            this.show=true
    ...
    changeRadio2(){
      ...
      if(this.show){
        let data={
          ...
          loginUserId: 1,
          detailName:'充值交费',
          // redirectUrl:'http://h5prod.apsatcom.com',//开发
        ...
        this.$store.state.accountId = this.$store.state.user.username
  ...
  created(){
    ...
    this.number=this.$store.state.orderNo
    this.name=this.$store.state.packageName
    this.username=this.$store.state.user.username
    this.getPaymentDetail(this.$store.state.acctId)
  }
}
</script>
